...
module.exports = function updateOneRecord (req, res) {
  ...
  var values = findTags_arr(actionUtil.parseValues(req));
  ...
  Model.findOne(pk).populateAll().exec(function found(err, matchingRecord) {
    ...
    matchingRecord = cleanAnswears(matchingRecord)
  ...
  function findTags_arr(values) {
    if(values.tags_arr){
      values.tags = _.without(values.tags_arr).toString();
    }  
    if(values.questions_arr){
      values.questions = _.without(values.questions_arr).toString();
    }
    return values;
  }
